...
pub(crate) async fn spawn_workspace_session(
    ...
    spawn_workspace_session_inner(entry, default_codex_bin, client_version, event_sink, codex_home)
        .await
}
...
pub(crate) async fn send_user_message(
    ...
    let policy = approval_policy.unwrap_or_else(|| "on-request".to_string());
    let policy = if access_mode == "full-access" {
        "never".to_string()
    } else {
        policy
    };
